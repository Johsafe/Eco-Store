...
const fetchProducts = async() =>{
  ...
  try {
    const fetched = await fetch(`http://localhost:5000/api/products/slug/${slug}`);
    ...
    <div>
      <Helmet>
        ...
        <div className={classes.detailDescr}>
          ...
          status:
          {product.inStock > 0 ?(
          <p style={{backgroundColor:'green',color:'white', borderRadius:"5px" ,padding:'2px 6px'}}>In Stock</p>
            ):(
          <p style={{backgroundColor:'red',color:'white', borderRadius:"5px" ,padding:'2px 6px'}}>
            Out Of Stock
           </p>)}
        </div>
